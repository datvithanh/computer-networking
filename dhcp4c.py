...
# ip = IPv6(dst="ff02::1:2", src=RandIP6())
...
class DHCPC_Am(AnsweringMachine):
    function_name = "dhcpv6 client"
    filter = "udp port 546 and port 547"
    send_function = staticmethod(send)

    @property
    def dhcp6_complete(self):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__xid = random.randint(0, sys.maxsize)

    def start_discover(self):
        if self.__discoverer is None:
            print("Sending discover with mac: {mac} through {iface}".format(
                mac=str2mac(self.__mac), iface=conf.iface))
            ether = Ether(dst='ff:ff:ff:ff:ff:ff',
                        src=self.__mac, type=0x0800)
            ...
            print("asdkk")
            self.__discoverer = TimedFunct(
                5, sendp, repetitions=5, args=[packet])
    ...
    def make_reply(self, req):
        ...
        self.__xid = random.randint(0, 4294967295)
